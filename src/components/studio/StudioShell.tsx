...
import { pouchDielineOverlay } from "@/lib/configurator/pouch-geometry";
...
export function StudioShell({ config, catalogue }: StudioShellProps) {
  ...
  const exportPreview = useCallback(() => {
    ...
    a.href = url;
    a.download = `${config.id}-preview.png`;
    a.click();
  }, [config.id]);
  ...
  const dieline = useMemo(() => {
    ...
    if (config.family === "folded-carton") {
      ...
      return dielineOverlay(spec, surface.editorWidth, surface.editorHeight);
    }
    if (config.pouchSpecId) {
      const spec = POUCHES[config.pouchSpecId ?? ""];
      if (!spec) return undefined;
      return pouchDielineOverlay(spec, surface.editorWidth, surface.editorHeight);
    }
    return undefined;
  }, [config, surface]);

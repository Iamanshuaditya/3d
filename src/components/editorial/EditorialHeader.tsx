...
export function EditorialHeader() {
  return (
    <header className="relative z-40 flex min-h-16 items-center justify-between gap-6 border-b border-st-line bg-st-bg px-5 sm:px-8">
      ...
      <nav aria-label="Main navigation" className="mr-auto hidden items-center gap-8 text-[13px] md:flex">
        {links.map((link) => <Link key={link.href} href={link.href} className="underline-offset-8 hover:underline">{link.label}</Link>)}
      </nav>
      <div className="hidden items-center gap-7 md:flex">
        <Link href="/designs" className="text-[13px] underline-offset-8 hover:underline">My designs</Link>
      ...
      <details className="group md:hidden">
        ...
        <nav aria-label="Mobile navigation" className="absolute left-0 right-0 top-full flex flex-col border-b border-st-line bg-st-bg px-6 py-5 shadow-lg">

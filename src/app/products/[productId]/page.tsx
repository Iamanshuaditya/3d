...
export default async function ProductDetailPage({params, searchParams}: {
  ...
  return <div className="editorial-page"><EditorialHeader /><main className="mx-auto max-w-[1600px] px-6 pb-20 pt-8 sm:px-12 lg:px-16">
    <nav aria-label="Breadcrumb" className="mb-12 flex flex-wrap gap-3 text-xs text-st-dim"><Link href="/">Home</Link><span>›</span><Link href="/products">Products</Link><span>›</span><span aria-current="page">{product.name}</span></nav>
    <div className="grid gap-12 lg:grid-cols-[1.05fr_1fr] lg:gap-20">
      <div className="relative min-h-[420px] bg-[#E7DDCB] sm:min-h-[620px]"><Image src={product.hero} alt={`${product.name} packaging inspiration`} fill priority sizes="(min-width:1024px) 50vw, 100vw" className="object-cover" /><div className="absolute bottom-6 left-6 bg-st-bg/95 p-5"><p className="text-[9px] uppercase tracking-wider text-st-dim">Packaging inspiration</p><p className="mt-2 font-display text-xl">{product.name}</p><p className="mt-1 text-xs text-st-dim">Your artwork. Every detail.</p></div></div>
      <section className="self-center"><p className="mb-4 text-[11px] uppercase tracking-[0.15em] text-[#A75232]">{product.family}</p><h1 className="font-display text-[clamp(44px,4.5vw,64px)] font-light leading-[1.05]">{product.name}</h1><p className="mb-8 mt-6 text-[15px] leading-7 text-st-dim">{product.description}</p><h2 className="mb-3 text-[10px] uppercase tracking-[0.12em] text-st-dim">Supported outputs — this product</h2><div className="mb-8 flex flex-wrap gap-2">{product.outputs.map(output => <span key={output} className="editorial-tag">{output}</span>)}</div>
    ...
    <section className="mt-20 border-t border-st-line pt-9"><h2 className="mb-6 text-xs uppercase tracking-wider text-st-dim">Other products</h2><div className="grid gap-6 sm:grid-cols-2">{products.filter(item => item.id !== productId).map(item => <Link key={item.id} href={`/products/${item.id}`} className="group flex items-center gap-5 border border-st-line p-4"><Image src={item.image} alt={item.name} width={96} height={80} className="h-20 w-24 object-cover"/><div><h3 className="font-display text-2xl">{item.name}</h3><p className="mt-1 text-xs text-st-dim">{item.subtitle}</p></div><ArrowRight className="ml-auto h-5 w-5 group-hover:translate-x-1"/></Link>)}</div></section>

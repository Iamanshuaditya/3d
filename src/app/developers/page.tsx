...
export default function DevelopersPage() {
  return <main className="editorial-page mx-auto w-full max-w-6xl px-5 py-8 sm:px-8 sm:py-12">
    <nav aria-label="Developer navigation" className="flex flex-wrap items-center justify-between gap-4 border-b border-[var(--st-line)] pb-6">
      <Link href="/" className="editorial-back inline-flex items-center gap-2 text-sm text-[var(--st-dim)]"><ArrowLeft className="h-4 w-4" /> Product library</Link>
      <a href="/integration.md" download className="editorial-link inline-flex items-center gap-2 text-sm font-medium"><Download className="h-4 w-4" /> Download guide</a>
    </nav>
    <header data-rise className="grid gap-8 py-12 sm:py-16 lg:grid-cols-[1.5fr_1fr] lg:items-end">
      ...
      <div>
        ...
        <Link href="/developers/demo" className="editorial-pill mt-6 inline-flex min-h-12 items-center gap-6 rounded-full bg-[var(--st-text)] px-5 text-sm font-medium text-white hover:bg-black">Try the API live <ArrowUpRight className="h-4 w-4" /></Link>
    ...
    <p data-reveal="up" className="rounded-xl border border-[var(--st-line)] bg-[var(--st-raised)] px-5 py-4 text-sm leading-relaxed">Before you start: get your Vortex host URL and server API key, and register your website’s exact origin. Keep the key on your backend.</p>
    <div className="divide-y divide-[var(--st-line)]">
      ...
      ].map((step) => <section key={step.label} data-stagger className="grid gap-6 py-10 lg:grid-cols-[0.7fr_1.3fr] lg:gap-12">
        <div data-reveal="up"><p className="font-mono text-xs text-[var(--st-dim)]">{step.label}</p><h2 className="mt-3 text-2xl font-medium tracking-tight">{step.title}</h2><p className="mt-3 text-sm leading-relaxed text-[var(--st-dim)]">{step.detail}</p></div>
        <pre data-reveal="up" className="min-w-0 overflow-auto overscroll-contain rounded-2xl bg-[#17191c] p-5 text-xs leading-[1.8] text-[#e2e3e6] sm:p-6"><code>{step.code}</code></pre>
      </section>)}
    </div>
    <section data-stagger className="border-t border-[var(--st-line)] py-10">
      <h2 data-reveal="up" className="text-2xl font-medium tracking-tight">Start with these products</h2>
      <div data-reveal="up" className="mt-5 overflow-x-auto overscroll-x-contain"><table className="w-full text-left text-sm">
      ...
      <p data-reveal="up" className="mt-4 max-w-3xl text-sm leading-relaxed text-[var(--st-dim)]">A generated PDF is not manufacturer certification. Review the returned print notes with your printer. The coffee cup exports a rectangular artwork area; it is not a curved cup manufacturing dieline. The original pouch returns <code>saved</code>, with no PDF.</p>
    </section>
    <section data-stagger className="grid gap-6 border-t border-[var(--st-line)] py-10 text-sm sm:grid-cols-2">
      <div data-reveal="up"><h2 className="font-semibold">Continue an existing design</h2><p className="mt-2 leading-relaxed text-[var(--st-dim)]">Create another session with the same <code>productId</code> and <code>resumeSessionId</code>. The saved project and product version are preserved. The previous editor URL stops working.</p></div>
      <div data-reveal="up"><h2 className="font-semibold">Know when it’s ready</h2><p className="mt-2 leading-relaxed text-[var(--st-dim)]"><code>editing</code> → <code>ready</code> means a PDF exists for the current saved revision. Preview sessions return <code>saved</code>. An edit after completion requires finishing again.</p></div>
    </section>
    <footer data-reveal="up" className="flex flex-wrap items-center justify-between gap-4 border-t border-[var(--st-line)] py-6 text-sm text-[var(--st-dim)]"><span>Vortex Studio · Editor API v1</span><a href="/integration.md" download className="editorial-link font-medium text-[var(--st-text)]">Get the complete short guide →</a></footer>
  </main>;
}
